...
public class SolutionObject{
    ...
    // By default
    public SolutionObject(List<Vehicle> vehicles, TaskSet tasks) {
    ...
    public double getTotalCost(boolean isToPrint){
        double MaxTimeCost = 0.0;
        double timeCostMax = Double.MIN_VALUE;
        double worldDistanceCost = 0.0;
        double speedMean = 0.0;
        ArrayList<Double> worldTime = new ArrayList<>();
        ...
        for(Vehicle vehicle: vehicles) {
            ...
            double timeCost = distance / vehicle.speed(); //Time of the whole plan
            worldDistanceCost += distance*vehicle.costPerKm(); // Cost of the distance of all vehicles

            worldTime.add(timeCost);
            if (timeCost > timeCostMax){
                MaxTimeCost = timeCost;
                timeCostMax = timeCost;
            }
            speedMean += vehicle.speed();
        }
        speedMean = speedMean / vehicles.size();

        if (isToPrint){
            System.out.println("Time to achieve the plan: " + MaxTimeCost);
            System.out.println("Distance cost of all vehicles to achieve the plan: " + worldDistanceCost);
