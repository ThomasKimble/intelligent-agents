...
public class CentralizedTemplate2 implements CentralizedBehavior {
...
//		ArrayList<Plan> plans = null;
	...
	public Solution SLS(List<Vehicle> vehicles, TaskSet tasks, double P, String initMethod) {
		...
		Solution Ainitial = selectInitialSolution(vehicles, tasks, initMethod);
		Solution Acurrent = new Solution(Ainitial);
		Solution Abest = new Solution(Ainitial);

		// ***** 2nd step : Repeat until termination condition met *****
		boolean stop = false;
		int noImprovementCounter = 0;
		int noImprovementBestCounter = 0;
		int numberOfAbest = 0;
		int totalIterations = 0;

		while (!stop) {
			// ** 1st sub-step : ChooseNeighbours **
			// This function provides a set of candidate assignment that are close to the
			// current one and could possibly improve it.
			ArrayList<Solution> N = chooseNeighboors(Acurrent, vehicles);

			// ** 2nd sub-step : LocalChoice **
			// The probability p is a parameter of the algorithm. If p is close to 1, the
			// algorithm converges faster but it is easily trapped into a local minimum.
			Acurrent = localChoice(vehicles, N, Acurrent, P);

			// ** 3rd sub-step : Check conditions to update bestSolution and to start **
			// **                again a sub-part or start the next one.              ** 
			// Save this sub-solution if this is the best one.
			
			if (getCost(Acurrent, vehicles) < getCost(Abest, vehicles)) {
				Abest = new Solution(Acurrent);
				noImprovementCounter = 0;
			}
			
			// Start again the sub-part if there is no improvement of the cost of the 
			// sub-solution found.
			if (noImprovementCounter == 10000) { // CRITERIA 1
				Acurrent = new Solution(Ainitial);
				noImprovementCounter = 0;
				numberOfAbest++; 
			}	
			
			// Switch to the next part if we have computed this sub-part enough and check 
			// if there is an improvement of the bestSolution since last parts.
			if (numberOfAbest == 3) { // CRITERIA 2
				double previousBestCost = getCost(Acurrent, vehicles);
				double currentBestCost = getCost(Abest, vehicles);
				if (currentBestCost == previousBestCost)
					noImprovementBestCounter++;
				else
					noImprovementBestCounter = 0;
				
				Ainitial = new Solution(Abest);
				Acurrent = new Solution(Ainitial);
				numberOfAbest = 0;
			}
			
			//** 4th sub-step : Repeat until termination condition met **
			if (noImprovementBestCounter == 3) { // CRITERIA 3
				stop = true;
			}
